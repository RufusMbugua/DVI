	...
	<tbody>
		...
		<tr>
			...
			<input type="text" name="email" id="email"  class="validate[required,custom[email]]" value="<?php echo $email_address;?>"/>
				...
				<tr>
			<td  width="5">Phone Number &nbsp;<span class="mandatory">*	</span>  			
			</td>
			
			<td>
		
			<select id="country_code" name="country_code"  style="display:block">
					
				<option value="chose">Choose Country </option>
				...
				foreach ($country as $countryS)
				 {
		            ...
		            echo '<option  value="'. $countryS['extention'].'">'. $countryS['Country'].' ('. $countryS['extention'].') </option>';
					$code= $countryS['extention'];
				    
					}
							
				?>
</select><input type="text" name="number" id="number"  maxlength="9" class="validate[required]"  style="display:block"  value="<?php echo $number=substr($number, -9,9); ?>"/>
